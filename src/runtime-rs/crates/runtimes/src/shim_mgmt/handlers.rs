...
mod tests {
    ...
    #[test]
    fn test_metrics_scrape_limit_rejects_concurrent_scrapes() {
        let first = METRICS_SCRAPE_LIMIT.try_acquire().unwrap();
        assert!(METRICS_SCRAPE_LIMIT.try_acquire().is_err());

        drop(first);
        assert!(METRICS_SCRAPE_LIMIT.try_acquire().is_ok());
    }
}
